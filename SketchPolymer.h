...
public:
    ...
    Stage1(uint32_t memory, uint32_t dim): d(dim) {
        m = memory * 1024 / sizeof(uint32_t) / d;
        counter = new uint32_t* [d];
        for (int i = 0; i < d; ++i) {
            counter[i] = new uint32_t [m];
            memset(counter[i], 0, m * sizeof(uint32_t));
        }
    }
    ~Stage1() {
        for (int i = 0; i < d; ++i) {
...
public:
    ...
    threshold(t), log_base(a) {
        stage1 = new Stage1<ID_TYPE>(0.05 * memory, d1);
        stage2 = new Stage2<ID_TYPE>(0.3 * memory, d2);
        stage3 = new Stage3<ID_TYPE>(0.5 * memory, d3);
        stage4 = new Stage4<ID_TYPE>(0.15 * memory, d4);
    }
